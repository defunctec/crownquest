...
public class  CrownQuest extends JavaPlugin {
    ...
    public final static String CROWNQUEST_ENV = System.getenv("CROWNQUEST_ENV") != null ? System.getenv("CROWNQUEST_ENV") : "development";
    ...
    public final static String CROWN_NODE_HOST = System.getenv("CROWN_NODE_HOST") != null ? System.getenv("CROWN_NODE_HOST") : "localhost";
    public final static int CROWN_NODE_PORT = System.getenv("CROWN_NODE_PORT") != null ? Integer.parseInt(System.getenv("CROWN_NODE_PORT")) : 18332;
    ...
    public final static String CROWN_NODE_USERNAME = System.getenv("CROWN_NODE_USERNAME");
    public final static String CROWN_NODE_PASSWORD = System.getenv("CROWN_NODE_PASSWORD");
    ...
    public final static String SERVER_NAME=System.getenv("SERVER_NAME") != null ? System.getenv("SERVER_NAME") : "CrownQuest";
    ...
    public final static String STATSD_PREFIX = System.getenv("STATSD_PREFIX") != null ? System.getenv("STATSD_PREFIX") : "crownquest";
    ...
    public void onEnable() {
        ...
        // loads world wallet
        wallet=new Wallet(this, "crownquest_market");
        try {
            getBlockChainInfo();
        ...
        sendDiscordMessage("crownquest started");
    }
...
if (CrownQuest.REDIS.get("currency"+player.getUniqueId().toString()).equalsIgnoreCase(CrownQuest.DENOMINATION_NAME)){
        ...
        user.wallet.getBalance(0, new Wallet.GetBalanceCallback() {
            ...
            public void run(Long balance) {
                ...
                Score score = walletScoreboardObjective.getScore(ChatColor.GREEN + CrownQuest.DENOMINATION_NAME); //Get a fake offline player
  ...
  }//end CrownQuest.DENOMINATION_NAME here start Emerald scoreboard by @BitcoinJake09
  else if (CrownQuest.REDIS.get("currency"+player.getUniqueId().toString()).equalsIgnoreCase("emerald")){ 
  ...
  Score score = walletScoreboardObjective.getScore(ChatColor.GREEN + "Ems:"); //Get a fake offline player
    ...
    public void teleportToSpawn(Player player) {
        if (!player.hasMetadata("teleporting")) {
            ...
            location.setX(location.getX()+CrownQuest.rand(0,64)-32);
            location.setZ(location.getZ()+CrownQuest.rand(0,64)-32);
...
//        }, 0, 120L);
    ...
    public void recordMetric(String name,int value) {
        if(SERVER_NAME!=null) {
            statsd.gauge("crownquest."+SERVER_NAME+"."+name,value);
        }
    ...
    public void setTotalExperience(Player player) {
        ...
        if(CrownQuest.REDIS.exists("experience.raw."+player.getUniqueId().toString())) {
            rawxp=Integer.parseInt(CrownQuest.REDIS.get("experience.raw."+player.getUniqueId().toString()));
        }
  ...
  tempchunk="netherchunk";
      ...
      final String chunkName = tempchunk;
                        ...
                        BukkitScheduler scheduler = Bukkit.getServer().getScheduler();
                        final CrownQuest bitQuest = this;
                            ...
                            public void run(Long balance) {
                                try {
                                    if (balance >= LAND_PRICE) {
                                        if (user.wallet.move("land", CrownQuest.LAND_PRICE)) {

                                            CrownQuest.REDIS.set(chunkName+ "" + x + "," + z + "owner", player.getUniqueId().toString());
                                            CrownQuest.REDIS.set(chunkName+ "" + x + "," + z + "name", name);
                                            ...
                                            if (bitQuest.messageBuilder != null) {
                                                ...
                                                org.json.JSONObject sentCharge = bitQuest.messageBuilder.trackCharge(player.getUniqueId().toString(), CrownQuest.LAND_PRICE / 100, null);
                                            ...
                                            if (balance < CrownQuest.LAND_PRICE) {
                                                player.sendMessage(ChatColor.RED + "You don't have enough money! You need " +
                                                        ChatColor.BOLD + (int) Math.ceil((CrownQuest.LAND_PRICE - balance) / 100) + ChatColor.RED + " more Bits.");
                                            } else {
                                        ...
                                        player.sendMessage(ChatColor.RED + "You don't have enough money! You need " +
                                                ChatColor.BOLD + (int) Math.ceil((CrownQuest.LAND_PRICE) / 100) + ChatColor.RESET + ChatColor.RED + " Bits.");
                                    }
                                } catch (Exception e) {
      ...
      if (REDIS.get("currency"+player.getUniqueId().toString())==("emerald")){
        scheduler.runTask(this, new Runnable() {
                            ...
                            public void run() {
                                ...
                                try {
                                  //use remove emearlds to buy land
                                    if (((removeEmeralds(player,(int) Math.ceil((CrownQuest.LAND_PRICE) / 100))) == true)) {  
             
                                        CrownQuest.REDIS.set(chunkName+ "" + x + "," + z + "owner", player.getUniqueId().toString());
                                        CrownQuest.REDIS.set(chunkName+ "" + x + "," + z + "name", name);
                                        player.sendMessage(ChatColor.GREEN + "Congratulations! You're now the owner of " + name + "!");
                                        player.sendMessage(ChatColor.YELLOW + "Price was "+(CrownQuest.LAND_PRICE / 100)+" Emeralds");
              ...
              land_owner_cache=new HashMap();
                                        ...
                                        if (countEmeralds(player) < (CrownQuest.LAND_PRICE/100)) {
                                            player.sendMessage(ChatColor.RED + "You don't have enough Emeralds! You need " + ChatColor.BOLD + Math.ceil(((CrownQuest.LAND_PRICE/100) - countEmeralds(player))) + ChatColor.RED + " more emeralds.");
                                        } else {
                                            player.sendMessage(ChatColor.RED + "Claim payment failed. Please try again later.");
                                        }
                                    }
                        ...
                        if (name.equals("abandon")) {
                            // Abandon land
                            CrownQuest.REDIS.del(tempchunk+ "" + x + "," + z + "owner");
                            CrownQuest.REDIS.del(tempchunk+ "" + x + "," + z + "name");
          CrownQuest.REDIS.del(tempchunk+ "" + x + "," + z + "permissions");
                            ...
                            if (REDIS.exists("uuid:" + newOwner)) {
                                ...
                                CrownQuest.REDIS.set(tempchunk+ "" + x + "," + z + "owner", newOwnerUUID);
