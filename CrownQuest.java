...
public class CrownQuest extends JavaPlugin {
  ...
  public static final String CROWN_NODE_HOST =
      System.getenv("CROWN_NODE_HOST") != null ? System.getenv("CROWN_NODE_HOST") : null;
  public static final int CROWN_NODE_PORT =
      System.getenv("CROWN_NODE_PORT") != null
          ? Integer.parseInt(System.getenv("CROWN_NODE_PORT"))
          : 19340;
  ...
  public static final String CROWN_NODE_USERNAME = System.getenv("CROWN_NODE_USERNAME");
  public static final String CROWN_NODE_PASSWORD = System.getenv("CROWN_NODE_PASSWORD");
  ...
  // Support for the crwcore full node and insight-api.
  ...
  public static final String STATSD_PREFIX =
      System.getenv("STATSD_PREFIX") != null ? System.getenv("STATSD_PREFIX") : "crownquest";
            ...
            public void run(Long balance) {
              ...
              Score score =
                  walletScoreboardObjective.getScore(
                      ChatColor.GREEN + CrownQuest.DENOMINATION_NAME); // Get a fake offline player

              score.setScore((int) (balance / DENOMINATION_FACTOR));
              player.setScoreboard(walletScoreboard);
            }
          });
    } //end CrownQuest.DENOMINATION_NAME here start Emerald scoreboard by @CrownJake09
...
scheduler.runTaskAsynchronously(this, new Runnable() {
  ...
  public void createPet(User user, String pet_name) {
    ...
    CrownQuest.REDIS.zincrby("player:tx", PET_PRICE, user.player.getUniqueId().toString());
  ...
  public void adoptPet(Player player, String pet_name) {
	...
	if ((user.countEmeralds()>= new Double(PET_PRICE / DENOMINATION_FACTOR).intValue())&&(CrownQuest.REDIS.get("currency"+player.getUniqueId().toString()).equalsIgnoreCase("emerald"))) { 
          player.getInventory().removeItem(new ItemStack[] {new ItemStack(Material.EMERALD, 100)});
        ...
        if (entity instanceof Ocelot) {
          if (entity.getCustomName() != null && entity.getCustomName().equals(cat_name)) {
            if (cat_is_found == false) {
              ...
              cat_is_found = true;
            } if (CrownQuest.REDIS.get("petIsOn"+player.getUniqueId().toString()).equalsIgnoreCase("off")) {
	    entity.remove();
	    } else {
              entity.remove();
            }
      ...
      if (!hasNonAlpha) {
        ...
        if (name.length() <= 27) {
          ...
          if (REDIS.get(tempchunk+""+ x + "," + z + "owner") == null) {
            ...
            if ((CROWN_NODE_HOST != null)&&(CrownQuest.REDIS.get("currency"+player.getUniqueId().toString()).equalsIgnoreCase(CrownQuest.DENOMINATION_NAME))) {
              user.wallet.getBalance(
                  0,
                  new Wallet.GetBalanceCallback() {
                    @Override
			...
			if (player.getLocation().getWorld().getName().equals("world_nether"))
			 { landxprice=4;}
                        if (balance >= (LAND_PRICE*landxprice)) {
                          if (user.wallet.move("land", (LAND_PRICE*landxprice))) {
                            saveLandData(player, name, x, z);

                          } else {
                            if (balance < (CrownQuest.LAND_PRICE*landxprice)) {
                              player.sendMessage(
                                  ChatColor.DARK_RED
                                      ...
                                      + (int)
                                          Math.ceil(
                                              ((CrownQuest.LAND_PRICE*landxprice) - balance)
                                                  / CrownQuest.DENOMINATION_FACTOR)
                                      + ChatColor.DARK_RED
                                      + " more "
                          ...
                          player.sendMessage(
                              ChatColor.DARK_RED
                                  ...
                                  + (int)
                                      Math.ceil(
                                          (CrownQuest.LAND_PRICE*landxprice) / CrownQuest.DENOMINATION_FACTOR)
                                  + ChatColor.DARK_RED
                                  + CrownQuest.DENOMINATION_NAME);
                        }
                      } catch (Exception e) {
			...
			if (player.getLocation().getWorld().getName().equals("world_nether"))
			 { landxprice=4;}
              int land_price_in_emeralds = (int) ((LAND_PRICE*landxprice) / CrownQuest.DENOMINATION_FACTOR);
            ...
            if (name.equals("abandon")) {
              ...
              if (REDIS.exists("uuid:" + newOwner)) {
                ...
                player.sendMessage(
                    ChatColor.GREEN + "This land now belongs to " + ChatColor.BLUE + newOwner);
              } else {
                player.sendMessage(
                    ChatColor.DARK_RED
                        + "Could not find "
